function [psnr_value, ssim_value] = embed_rec_y_func(org_img_path, output_dir)
% EMBED_REC_Y_FUNC - 水印嵌入函数
% 输入:
%   org_img_path - 原始图像路径
%   output_dir - 输出目录路径
% 输出:
%   psnr_value - 嵌入后图像的PSNR值
%   ssim_value - 嵌入后图像的SSIM值
...
% 获取输出文件名，统一使用.png扩展名
[img_path, img_name, img_ext] = fileparts(org_img_path);
output_img_path = fullfile(output_dir, [img_name '.png']);
...
% 读取原始图像
[img_raw, MAP] = imread(org_img_path);

% 计算JND图
jnd_map = compute_color_jnd_1024_rec_y(org_img_path);
...
%% 七、保存结果（适配单通道/三通道）
% 只保存嵌入水印后的图像到目标目录
imwrite(uint8(final_embed_img * 255), output_img_path);
...
% 保存中间结果（包含哈达玛参数）到单独的目录
intermediate_dir = './watermark_intermediates';
if ~exist(intermediate_dir, 'dir')
    mkdir(intermediate_dir);
end
result_filename = [img_name '_watermark_final_result_hadamard.mat'];
save(fullfile(intermediate_dir, result_filename), 'diff_original', 'rgb_jnd_map', 'final_embed_img', ...
     'process_channel_sum', 'img_org', 'pixel_diff_map', 'process_channel_sum_after', 'model1', 'code1', 'img_c', 'H', 'hadamard_weights');
fprintf('中间结果已保存至：%s\n', fullfile(intermediate_dir, result_filename));
