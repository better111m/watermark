function [extraction_rate_64, extraction_rate_39] = extract_rec_y_func(watermarked_img_path, original_watermark_path)
% EXTRACT_REC_Y_FUNC - 水印提取函数
% 输入:
%   watermarked_img_path - 嵌入水印后的图像路径
%   original_watermark_path - 原始水印路径（用于计算准确率）
% 输出:
%   extraction_rate_64 - 64位水印提取准确率
%   extraction_rate_39 - 39位水印提取准确率
...
% 路径配置
optimized_img_path = watermarked_img_path;
...
loaded_model = load(model1_path, 'model1');
...
if exist(bch_param_path, 'file')
    loaded_data = load(bch_param_path);
    if isfield(loaded_data, 'code1') && all(ismember(loaded_data.code1, [0, 1]))
        gen_poly = loaded_data.code1;
        fprintf('✅ 加载BCH生成多项式\n');
    else
        error('code1.mat格式错误！');
    end
...
if exist(original_watermark_path, 'file') && ~isempty(final_watermark)
    loaded_original = load(original_watermark_path, 'original_watermark');
    original_watermark = loaded_original.original_watermark;
...
% 返回两个提取准确率
extraction_rate_64 = model1_accuracy;
extraction_rate_39 = orig_wm_accuracy;

% 保存中间结果到指定文件夹
[img_path, img_name, img_ext] = fileparts(watermarked_img_path);
...
result_filename = fullfile(extraction_results_dir, [img_name '_extraction_result.mat']);
